...
public class RegistrationTests extends BaseTest {
    ...
    void test1() {
        TokenDto tokenDto = given()//Начало запроса
                ...
                .body(getRegistrationRq())// Тело запроса (POST & PUT) ------------сюда бади с контактом или логины
                .when()// с каким методом и на какой эндпойнт отправляем запрос.
                .log().all()// логирование в консоль
                .post(REGISTRATION_ENDPOINT)// на какой адрес (или endpoint) отправляем запрос --------- эндрпоинт
    ...
    void test2() {
        TokenDto tokenDto = given()//Начало запроса
                .contentType(ContentType.JSON)//Указываем тип контента
                .body(getRegistrationNegRq())// Тело запроса (POST & PUT) ------------сюда бади с контактом или логины
                .when()// с каким методом и на какой эндпойнт отправляем запрос.
                .log().all()// логирование в консоль
                .post(REGISTRATION_ENDPOINT)// на какой адрес (или endpoint) отправляем запрос --------- эндрпоинт
                .then()// Что делаем когда получаем ответ
                .log().all()//Логируем ответ
                .assertThat().statusCode(409)//Проверяем статус код пришедшего ответа
                .extract().response().as(TokenDto.class);//извлекаем ответ.
